...
Каждый вызов в цепочке создаёт [замыкание](/frontend/javascript/ch01-closures), которое захватывает переданный аргумент:
...
`add(5)` возвращает новую функцию, замкнутую на `x = 5`. Эта функция живёт, пока на неё есть ссылка, — точно как описано в главе про [замыкания](/frontend/javascript/ch01-closures/02-how-it-works).
...
Каррированные функции принимают по одному аргументу — их легко выстраивать в цепочки. Подробнее — в разделе [compose и pipe](/frontend/javascript/ch05-currying/03-partial-and-compose).
