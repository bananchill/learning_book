...
> Формальное описание этого механизма — в [спецификации ECMAScript](https://tc39.es/ecma262/#sec-lexical-environments). Более доступное объяснение — на [javascript.info](https://javascript.info/closure).
...
Детальный разбор scope chain есть в книге [You Don't Know JS: Scope & Closures](https://github.com/getify/You-Dont-Know-JS/tree/2nd-ed/scope-closures) — главы 1–3 посвящены именно этому. Рассмотрим пошагово, что происходит при выполнении:
...
Движок V8 не хранит в замыкании все переменные внешнего окружения. При парсинге кода V8 анализирует, какие переменные реально используются внутренними функциями, и выделяет только их в специальный объект `Context` на heap — остальные остаются на стеке и освобождаются при выходе из функции.

Подробности — в [статье V8 о lazy parsing](https://v8.dev/blog/preparser), а глубокий технический разбор с примерами машинного кода — в [Grokking V8 closures](https://mrale.ph/blog/2012/09/23/grokking-v8-closures-for-fun.html) от бывшего инженера V8.
