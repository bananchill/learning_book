...
Условные типы работают как тернарный оператор, но для типов. Синтаксис `T extends U ? X : Y` проверяет, является ли `T` подтипом `U`, и возвращает один из двух типов в зависимости от результата. Это позволяет создавать типы, которые адаптируются к входным данным:
...
Когда условный тип применяется к union-типу, он **распределяется** по каждому члену. Это значит, что TypeScript применяет условие к каждому элементу объединения отдельно, а затем собирает результаты обратно в union. Дистрибутивность включается автоматически, если параметр типа стоит «голым» (без обёрток вроде `[T]`) в позиции `extends`. Чтобы отключить такое поведение, достаточно обернуть `T` в кортеж:
...
`infer` позволяет **извлекать** типы из других типов в условных выражениях. Вместо того чтобы заранее знать конкретный тип, вы просите TypeScript «вывести» его из структуры. Ключевое слово `infer` работает только внутри ветки `extends` условного типа и вводит новую типовую переменную, которую компилятор заполнит автоматически:
...
TypeScript предоставляет набор готовых утилитарных типов, построенных на `infer`. Они покрывают самые частые сценарии извлечения типов из функций и классов:
...
Условные типы могут ссылаться сами на себя, создавая рекурсивные определения. Это полезно, когда нужно «развернуть» вложенные структуры произвольной глубины — например, распаковать массив массивов или цепочку промисов:
