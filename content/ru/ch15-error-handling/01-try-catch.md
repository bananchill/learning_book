...
Исключение (exception) — это сигнал о том, что в программе произошла ошибка, которую текущий код не может обработать самостоятельно. Без обработки ошибок одна неудачная операция — некорректный JSON, недоступный сервер, обращение к несуществующему свойству — приводит к аварийному завершению всей программы. Конструкция `try/catch/finally` позволяет перехватить исключение, обработать его (показать сообщение пользователю, вернуть значение по умолчанию) и гарантировать выполнение завершающего кода.
