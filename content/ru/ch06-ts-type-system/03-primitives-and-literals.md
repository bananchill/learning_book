...
Не путайте примитивные типы (`string`, `number`) с объектами-обёртками (`String`, `Number`). Тип `String` (с большой буквы) — это объект-обёртка, его почти никогда не нужно использовать в аннотациях типов.
</Callout>
...
<Callout type="tip">
Предпочитайте union литералов вместо enum для простых перечислений. Union не создаёт рантайм-объект и более идиоматичен для TypeScript.
</Callout>
...
<DeepDive title="Почему any нарушает иерархию">
...
<Callout type="warning">
Числовые enum-ы создают обратный маппинг (reverse mapping) и могут вести себя неожиданно. Предпочитайте строковые enum-ы или union литералов: `type Status = 'active' | 'inactive' | 'pending'`.
</Callout>
