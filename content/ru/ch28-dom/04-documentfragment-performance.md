...
Reflow (перекомпоновка) -- это процесс, при котором браузер заново вычисляет размеры и положение элементов на странице. Repaint (перерисовка) -- это повторная отрисовка пикселей на экране без изменения раскладки (например, смена цвета фона). Reflow всегда дороже repaint, поскольку изменение геометрии одного элемента может затронуть расположение всех остальных. Особенно опасен «forced reflow» -- когда браузер вынужден синхронно пересчитать раскладку прямо в момент чтения геометрического свойства вроде `offsetHeight` или `getBoundingClientRect()`.
...
Функция `requestAnimationFrame` (rAF) просит браузер выполнить переданный колбэк перед следующей отрисовкой кадра. Это позволяет собрать все DOM-изменения и применить их в один момент, синхронизированный с циклом рендеринга браузера (обычно 60 раз в секунду). Благодаря этому множественные изменения стилей и позиций объединяются в один reflow вместо того, чтобы вызывать пересчёт раскладки по отдельности.
