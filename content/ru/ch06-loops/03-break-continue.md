...
Иногда цикл нужно прервать досрочно (найден нужный элемент) или пропустить текущую итерацию (элемент не подходит). Для этого JavaScript предоставляет `break` и `continue`.

## break — выход из цикла

`break` немедленно завершает цикл и передаёт управление коду после него. Это ключевой инструмент оптимизации: зачем проверять оставшиеся 999 элементов, если нужный уже найден?
...
`continue` пропускает остаток текущей итерации и переходит к следующей. Это удобная альтернатива обёртке в `if` — вместо вложенного блока вы отсеиваете неподходящие элементы в начале:
