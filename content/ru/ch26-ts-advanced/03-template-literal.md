...
Template literal types по-настоящему раскрываются в паттерне генерации имён. Когда template literal комбинируется с union типом, TypeScript автоматически генерирует все возможные комбинации. Например, из списка событий `'click' | 'focus'` и шаблона `on${Capitalize<Events>}` получается union `'onClick' | 'onFocus'`. Это позволяет строить типизированные API для событий, геттеров, CSS-классов и любых других именованных сущностей, где имя следует предсказуемому шаблону.
...
Одно из практичных применений template literal types — валидация CSS-значений на этапе компиляции. Вместо обычного `string`, который допускает любую строку, мы конструируем тип из шаблона `${number}${CSSUnit}`, который принимает только корректные значения вроде `'16px'` или `'2rem'`. Опечатка или пропущенная единица измерения будет поймана TypeScript ещё до запуска кода.
...
Паттерн DeepKeys решает типичную задачу: как типизировать dot-notation доступ к вложенным свойствам объекта, например `'server.host'` или `'db.url'`. Тип рекурсивно обходит структуру объекта, конкатенируя имена ключей через точку с помощью template literal types. В результате получается union всех возможных путей, и TypeScript не позволит обратиться к несуществующему пути.
...
Template literal types работают не только на построение строк, но и на их разбор. Используя `infer` внутри шаблона, мы можем извлечь части строки по разделителю. Например, паттерн `` `${infer K}:${infer V}` `` разбивает строку `'name:Иван'` на `K = 'name'` и `V = 'Иван'`. Это фактически парсер строк на уровне системы типов, позволяющий типизировать URL, конфигурационные строки и DSL-выражения.
