...
Метод `addEventListener` — основной способ подписки на события в DOM. Он принимает имя события, функцию-обработчик и необязательный объект настроек. В отличие от устаревших свойств вроде `onclick`, `addEventListener` позволяет навешивать несколько обработчиков на одно событие и гибко управлять фазой перехвата:
...
Каждый обработчик получает объект события (`Event`), содержащий информацию о произошедшем действии. В зависимости от типа события объект предоставляет разные свойства — координаты мыши, нажатые клавиши, целевой элемент и методы управления всплытием:
