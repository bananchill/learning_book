...
В [предыдущей главе](/frontend/javascript/ch02-async) мы разобрали, как JavaScript обрабатывает тысячи асинхронных операций в одном потоке. Event loop — это конкурентность: один поток переключается между задачами. Но что делать, когда одного потока мало? Тяжёлые вычисления блокируют event loop, и никакой `async/await` не поможет.
...
1. [Конкурентность vs параллелизм](/frontend/javascript/ch03-parallelism/01-concepts)
2. [Web Workers](/frontend/javascript/ch03-parallelism/02-web-workers)
3. [SharedArrayBuffer и Atomics](/frontend/javascript/ch03-parallelism/03-shared-memory)
4. [Параллелизм в Node.js](/frontend/javascript/ch03-parallelism/04-node-parallelism)
5. [Паттерны и решения](/frontend/javascript/ch03-parallelism/05-patterns-and-decisions)
